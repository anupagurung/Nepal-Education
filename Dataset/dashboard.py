...
for i, level in enumerate(levels):
    df_level = map_data_df[map_data_df['level'] == level]
    fig_map_enrollment.add_trace(go.Scattermapbox(
        lat=df_level['lat'], lon=df_level['lon'], mode='markers',
        marker=go.scattermapbox.Marker(size=np.sqrt(df_level['enrollment']) / 10, color=COLOR_PALETTE['map_bubble'], sizemode='diameter'),
        hovertemplate='<b>%{text}</b><br>Enrollment: %{customdata:,.0f}<extra></extra>',
        text=df_level['province'], customdata=df_level['enrollment'],
